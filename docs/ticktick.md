...
TickTick is the least fussy of the lot. It accepts plain `http`, and it accepts
a bare IP address, so whatever address you already use will work. This was
confirmed by registering a numeric redirect URI in the Developer Center and
watching it save without complaint.
...
borrowing `localhost` for the setup the way Google needs.
